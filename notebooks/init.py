...
def model(phi1, a, b, c, d, x):
    """Dado a,b,c,d,x devuelve una parabola "corrida" para los puntos phi1: a + b*(phi1-x) + c*(phi1-x)**2 + d*(phi1-x)**3
    """
...
def init_ls(phi1, pmphi1, pmphi2, d, miembro, nwalkers, ndim):
    ...
    init = np.array([params_mu1[0], params_mu2[0], params_d[0], params_mu1[1], params_mu2[1], params_d[1], params_mu1[2], params_mu2[2], params_d[2], params_mu1[3], params_mu2[3], params_d[3], params_mu1[4], params_mu2[4], params_d[4], 0.2]) #HACK: f = 0.2 ~ miembro_PW.sum()/footprint.sum()
