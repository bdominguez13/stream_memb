...
def ln_prior(theta):#, mu, sigma, d_mean, e_dd, lim_unif):
    ...
    """
    a_mu1, a_mu2, a_d, b_mu1, b_mu2, b_d, c_mu1, c_mu2, c_d, d_mu1, d_mu2, d_d, x_mu1, x_mu2, x_d, f = theta
    
    # lp_a12 = multivariate_normal.logpdf(np.stack((a_mu1, a_mu2), axis=-1), mean=mu, cov=sigma)
    lp_a12 = log_multivariate_gaussian(np.stack((a_mu1, a_mu2), axis=-1), mu, sigma)
...
def lnlike_st(theta):
    ...
    model_mu1 = init.model(phi1.value, a_mu1, b_mu1, c_mu1, d_mu1, x_mu1)
    model_mu2 = init.model(phi1.value, a_mu2, b_mu2, c_mu2, d_mu2, x_mu2)
    model_d = init.model(phi1.value, a_d, b_d, c_d, d_d, x_d)
    ...
    ll_st = log_multivariate_gaussian(y.T, model.T, C_tot)
