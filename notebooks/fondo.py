...
def XD_minBIC(N, pmra_out, pmdec_out, d_out, C_pm_radec_out, e_d_out):
    ...
    C_pm_radec_out, e_d_out: Errores en movimientos propios en ar y dec y distancias de las estrellas del background
    ...
    return N_best_xd, BIC_xd


def fondo_xd(N_best_xd, pmra_out, pmdec_out, d_out, C_pm_radec_out, e_d_out):
    ...
    Xerr = np.zeros((len(pmra_out),3,3))
    Xerr[:,:2,:2] = C_pm_radec_out
    Xerr[:,2,2] = e_d_out**2
    
    N = np.array([N_best_xd])
...
def fondo(N_best_xd, pmra_out, pmdec_out, d_out):#, e_pmra_out, e_pmdec_out, e_d_out):
    """Calcula un modelo de mezcla de gaussianas (GMM) para las estrellas del background usando el mejor numero de gaussianas segun un modelo de extreme decovolution (XD).
